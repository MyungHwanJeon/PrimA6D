...
parser = argparse.ArgumentParser(
                                description='R6D')
parser.add_argument('-m', '--mode', dest='mode', default="trt16",
                    help='[trt16, trt32]')    
...
def thread_inference(obj_idx, gpu_idx, input_data, detection_data):
    ...
    ctx = cuda.Device(gpu_idx).make_context()
    
    cp_dev = cp.cuda.Device(gpu_idx)
    cp_dev.use()
    ...
    if args.mode == "trt16":
        ...
        segmentation_engine = trt_utils.load_engine("../trt_utils/converted_weight/Segmentation/trt16/engine/model_" + str(obj_idx) + "_S.engine")            
        segmentation_inputs, segmentation_outputs, segmentation_bindings, segmentation_stream = trt_utils.allocate_buffers(segmentation_engine)
        segmentation_context = segmentation_engine.create_execution_context()
        #print("load trt16 Segmentator of obj", obj_idx)

        prima6d_engine = trt_utils.load_engine("../trt_utils/converted_weight/PrimA6D++/trt16/engine/model_" + str(obj_idx) + "_P.engine")            
        prima6d_inputs, prima6d_outputs, prima6d_bindings, prima6d_stream = trt_utils.allocate_buffers(prima6d_engine)
        prima6d_context = prima6d_engine.create_execution_context()
        #print("load trt16 PrimA6D of obj", obj_idx)
    
    elif args.mode == "trt32":
    
        dtype = np.float32
    
        segmentation_engine = trt_utils.load_engine("../trt_utils/converted_weight/Segmentation/trt32/engine/model_" + str(obj_idx) + "_S.engine")            
        segmentation_inputs, segmentation_outputs, segmentation_bindings, segmentation_stream = trt_utils.allocate_buffers(segmentation_engine)
        segmentation_context = segmentation_engine.create_execution_context()
        #print("load trt32 Segmentator of obj", obj_idx)

        prima6d_engine = trt_utils.load_engine("../trt_utils/converted_weight/PrimA6D++/trt32/engine/model_" + str(obj_idx) + "_P.engine")            
        prima6d_inputs, prima6d_outputs, prima6d_bindings, prima6d_stream = trt_utils.allocate_buffers(prima6d_engine)
        prima6d_context = prima6d_engine.create_execution_context()
        #print("load trt32 PrimA6D of obj", obj_idx)                
        
    if segmentation_engine == None or prima6d_engine == None:
        ctx.pop()
        return       
    ...
    while(1):
        try:                 
            ...
            if detection_data.get_data(DATA_FIELD.MODEL_FLAG)[0] == True:                             
                ...
                np.copyto(segmentation_inputs[0].host, input_data.get_data(DATA_FIELD.IMAGE_INPUT).ravel())            

                output = trt_utils.do_inference_v2(
                                                    segmentation_context, 
                                                    bindings=segmentation_bindings,
                                                    inputs=segmentation_inputs, 
                                                    outputs=segmentation_outputs, 
                                                    stream=segmentation_stream
                                                    )
                                                    
                segmentation_pred = output[0].reshape((1, 2, 480, 640))
                segmentation_pred_gpu = cp.array(segmentation_pred)
                segmentation_pred_gpu = cp.argmax(segmentation_pred_gpu, axis=1)
                segmentation_pred = segmentation_pred_gpu.get()
                
                obj_mask_idx = np.nonzero(segmentation_pred.reshape((480, 640)))   
                 
                if obj_mask_idx[0].shape[0] != 0 and obj_mask_idx[1].shape[0] != 0:   
                    ...
                    prima6d_inp = blank_image.reshape((1, 64, 64, 3)).astype(dtype) / 255.
                    prima6d_inp = np.transpose(prima6d_inp, (0, 3, 1, 2))
                    np.copyto(prima6d_inputs[0].host, prima6d_inp.flatten())
            
                    output = trt_utils.do_inference_v2(
                                                        prima6d_context,
                                                        bindings=prima6d_bindings, 
                                                        inputs=prima6d_inputs,
                                                        outputs=prima6d_outputs, 
                                                        stream=prima6d_stream
                                                        )
            
                    primitive_out = np.transpose(output[0].reshape((9, 64, 64)), (1, 2, 0))
                    keypoint_out = output[1].reshape((42, 2))
                    uncertainty_out = output[2].reshape((3))
                    ...
                    confidence_norm = np.array([0, 0, 0], dtype=dtype)
                    ...
                    t_est = np.zeros((3), dtype=dtype)
                    R_est = np.eye(3, dtype=dtype)
                    
                    if valid_cnt > 0:                        

                        kp_3d_pts = np.zeros((9 + valid_cnt * 5, 4), dtype=dtype)
                        kp_2d_pts = np.zeros((9 + valid_cnt * 5, 2), dtype=dtype)
                        ...
                        detection_data.set_data(DATA_FIELD.SEGMENTATION, segmentation_pred.reshape((480, 640, 1)))
...
def thread_result_image(result_image_data, obj_idxs):
    ...
    #K = np.array([599.8289184570312, 0.0, 321.4010925292969, 0.0, 600.1539306640625, 242.77597045898438, 0.0, 0.0, 1.0]).reshape(3, 3) 
