...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        padding_obj_bb_xywh = make_padding_bb(img=cropped_obj_inp, bb_xywh=obj_bb_square_xywh, render_dimension=[obj_inp.shape[1], obj_inp.shape[0], obj_inp.shape[2]], pad_factor=1.4)
        ...
        primitive_x_gt = primitive_x_gt[int(top):int(bottom), int(left):int(right)]
        primitive_y_gt = primitive_y_gt[int(top):int(bottom), int(left):int(right)]
        primitive_z_gt = primitive_z_gt[int(top):int(bottom), int(left):int(right)]
        ...
        if padding_obj_bb_xywh[2] > result_image_size[0] or padding_obj_bb_xywh[3] > result_image_size[0]:
            ...
            primitive_x_gt = cv2.resize(primitive_x_gt, result_image_size, interpolation=cv2.INTER_AREA) 
            primitive_y_gt = cv2.resize(primitive_y_gt, result_image_size, interpolation=cv2.INTER_AREA) 
            primitive_z_gt = cv2.resize(primitive_z_gt, result_image_size, interpolation=cv2.INTER_AREA)             
            ...
            primitive_x_gt = cv2.resize(primitive_x_gt, result_image_size, interpolation=cv2.INTER_LINEAR_EXACT) 
            primitive_y_gt = cv2.resize(primitive_y_gt, result_image_size, interpolation=cv2.INTER_LINEAR_EXACT) 
            primitive_z_gt = cv2.resize(primitive_z_gt, result_image_size, interpolation=cv2.INTER_LINEAR_EXACT)                    
        ...
        kp_2d_gt = np.zeros((3, 28), dtype=np.float32)
        ...
        for iii in range(3):                     
            ...
            R_t[0:3, 3] = tra_gt
            P = np.matmul(K, R_t)
            kp_2d = np.matmul(P, np.transpose(self.kp_3d_4d[iii]))
            ...
            kp_2d_gt[iii] = kp_2d[:, 0:2].reshape(-1)                                                           
                
        obj_inp = self.ToTensor(obj_inp)        
        obj_gt = self.ToTensor(obj_gt)
        primitive_gt = self.ToTensor(primitive_gt)
        primitive_x_gt = self.ToTensor(primitive_x_gt)
        primitive_y_gt = self.ToTensor(primitive_y_gt)
        primitive_z_gt = self.ToTensor(primitive_z_gt)  
        ...
        if self.img_origin:
            if 'obj_depth' in data.keys():   
                return data["obj_inp"].astype(np.uint8), obj_inp, obj_gt, primitive_gt, primitive_x_gt, primitive_y_gt, primitive_z_gt, obj_bb, rot_gt, tra_gt, kp_2d_gt, K, obj_depth
            else:
                return data["obj_inp"].astype(np.uint8), obj_inp, obj_gt, primitive_gt, primitive_x_gt, primitive_y_gt, primitive_z_gt, obj_bb, rot_gt, tra_gt, kp_2d_gt, K, np.ones(1)
        else:
            if 'obj_depth' in data.keys():   
                return obj_inp, obj_gt, primitive_gt, primitive_x_gt, primitive_y_gt, primitive_z_gt, obj_bb, rot_gt, tra_gt, kp_2d_gt, K, obj_depth
            else:
                return obj_inp, obj_gt, primitive_gt, primitive_x_gt, primitive_y_gt, primitive_z_gt, obj_bb, rot_gt, tra_gt, kp_2d_gt, K, np.ones(1)
