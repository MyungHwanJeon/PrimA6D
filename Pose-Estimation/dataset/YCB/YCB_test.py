...
path = args.path + "/test/obj_" + str(args.obj)
os.makedirs(path, exist_ok=True)    
                            
def make_testset(obj_id):
    ...
    depth_list = []
    mask_list = []
    for nn in YCB_testset_index:
        
        f = open("./keyframe.txt")
        keyframes = f.readlines()
        nn_keyframes = []
        for i in range(len(keyframes)):
            sp = keyframes[i].split("/")
            if int(sp[0]) == nn:
                nn_keyframes.append(str(int(sp[1])))
        ...
        YCB_SCENE_GT_PATH = config.YCBV_dataset_path + "/ycbv_test/" + "%06d" % (nn) + "/scene_gt.json"
        YCB_GT_INFO_PATH = config.YCBV_dataset_path + "/ycbv_test/" + "%06d" % (nn) + "/scene_gt_info.json"    
        ...
        for i in nn_keyframes:
            for j in range(len(YCB_scene_gt[i])):       
            
                #if (YCB_gt_info[i][j]["visib_fract"] < 0.5):
                #    continue
            
                if (YCB_gt_info[i][j]["px_count_visib"] == 0):
                    continue
                
                if (YCB_gt_info[i][j]["bbox_visib"][2] <= 0):
                    continue
                    
                if (YCB_gt_info[i][j]["bbox_visib"][3] <= 0):
                    continue  
                         
                if YCB_scene_gt[i][j]["obj_id"] is obj_id:
                    rot_data.append(YCB_scene_gt[i][j]["cam_R_m2c"])
                    tra_data.append(YCB_scene_gt[i][j]["cam_t_m2c"])
                    bbox_data.append(YCB_gt_info[i][j]["bbox_obj"])
                    bbox_visib_data.append(YCB_gt_info[i][j]["bbox_visib"])                                                            
                    image_list.append(config.YCBV_dataset_path + "/ycbv_test/" + "%06d" % (nn) + "/rgb/" + "%06d" % (int(i)) + ".png")
                    depth_list.append(config.YCBV_dataset_path + "/ycbv_test/" + "%06d" % (nn) + "/depth/" + "%06d" % (int(i)) + ".png")
                    mask_list.append(config.YCBV_dataset_path + "/ycbv_test/" + "%06d" % (nn) + "/mask_visib/" + "%06d" % (int(i)) + "_" + "%06d" % (j) + ".png")
                    K_data.append(cam_K)
    ...
    for n in range(size):
        ...
        state, obj_color, obj_depth, obj_mask, gt_color, primitive_color, primitive_x_color, primitive_y_color, primitive_z_color = dataset.render_synthetic(np_rot_data[n], np_tra_data[n])  
        ...
        if state is True:
            ...
            f = gzip.open(path + "/obj_" + str(args.obj) + "_test_" + "%05d"%n + ".gz","wb")
